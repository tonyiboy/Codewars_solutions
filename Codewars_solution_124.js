...
function topSecret(str)  {
    const A_CODE_LW = "a".charCodeAt();
    const Z_CODE_LW = "z".charCodeAt();
    const A_CODE_UP = "A".charCodeAt();
    const Z_CODE_UP = "Z".charCodeAt();
    const SHIFT = -3;
    const NB_LETTERS_ALPHABET = 26;
  
    let decrypted = "";
    for (let i = 0; i < str.length; i++) {
        let c_enc = str[i].charCodeAt();
        let c_dec = "";
        let code = str[i].charCodeAt() + SHIFT;
      
        if (A_CODE_LW <= c_enc && c_enc <= Z_CODE_LW)    
            c_dec = String.fromCharCode(code >= A_CODE_LW ? code : code + NB_LETTERS_ALPHABET);
        else if (A_CODE_UP <= c_enc && c_enc <= Z_CODE_UP)
            c_dec = String.fromCharCode(code >= A_CODE_UP ? code : code + NB_LETTERS_ALPHABET);
        else
            c_dec = str[i];
